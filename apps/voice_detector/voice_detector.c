...
#define VAD_RAW_DATA_BUFF_SIZE (1024 * 16)
#else
...
static int voice_detector_vad_stop(struct voice_detector_dev *pdev)
{
...
#if defined(CHIP_BEST2300P)
    short *pbuf_short = (short *)pdev->vad_raw_data_buff;
#endif
    ...
    len = 0;
#else
    /* receive vad data after stopping it */
    len = af_vad_get_data(NULL, 0);
    TRACE(2,"%s: vad data len: %u", __func__, len);

    len = af_vad_get_data(pbuf, VAD_RAW_DATA_BUFF_SIZE);
#endif
