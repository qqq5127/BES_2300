...
/*cuixu add start 20210120*/
#define	ANC_FACTORY_STRING_LEN	17
bool app_anc_factory_test_flag = false;
#include "ui_event_manage.h"
#include "ui_event_scpi.h"

static void app_anc_factory_test_check(uint8_t* ptrData, uint32_t dataLength)
{
	
	const uint8_t anc_test_enable_string[ANC_FACTORY_STRING_LEN] = {"zowee anc test 1"};
	const uint8_t anc_test_disable_string[ANC_FACTORY_STRING_LEN] = {"zowee anc test 0"};
	const uint8_t resume_factory_test_string[] = {"zowee resume factory 1"};

	TOTA_LOG_DBG(1,"app_anc_factory_test_flag %d %s",dataLength,ptrData);
	if(dataLength < ANC_FACTORY_STRING_LEN)
	{
		return;
	}

	if(memcmp(ptrData,anc_test_enable_string,ANC_FACTORY_STRING_LEN-1) == 0)
	{
		TOTA_LOG_DBG(1,"app_anc_factory_test_flag ture");
		app_anc_factory_test_flag = true;
		app_tota_send_cmd_via_spp(ptrData,dataLength);
	}
	else if(memcmp(ptrData,anc_test_disable_string,ANC_FACTORY_STRING_LEN-1) == 0)
	{		
		TOTA_LOG_DBG(1,"app_anc_factory_test_flag false");
		app_anc_factory_test_flag = false;
		app_tota_send_cmd_via_spp(ptrData,dataLength);
	}
	else if(memcmp(ptrData,resume_factory_test_string,22) == 0)
	{
		TOTA_LOG_DBG(1,"app_resume_factory true");
		app_ui_event_process(UI_EVENT_SCPI,0,UI_EVENT_SCPI_SPP_FACTORY_MODE,0,0);
	}

}
/*cuixu add end 20210120*/
...
/*cuixu add start 20210120*/
    app_anc_factory_test_check(ptrData,dataLength);
/*cuixu add end 20210120*/
