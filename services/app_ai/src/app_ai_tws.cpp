...
bool app_ai_tws_role_switch_direct(void)
{
    ...
    {
        app_ibrt_if_tws_switch_prepare_done_in_bt_thread(IBRT_ROLE_SWITCH_USER_AI, app_ai_get_ai_spec());
        return ret;
    }
    else
    {
...
bool app_ai_tws_master_role_switch(void)
{
    ...
    {
        app_ai_if_thirdparty_event_handle(THIRDPARTY_FUNC_KWS, THIRDPARTY_AI_DISCONNECT);
    }

    app_ai_set_role_switching(true);
    osTimerStart(app_ai_role_switch_timer_id, APP_AI_ROLE_SWITCH_TIME_IN_MS);
    if (app_ai_is_stream_running() || (app_ai_get_speech_state() != AI_SPEECH_STATE__IDLE))
    {
...
void app_ai_tws_role_switch_complete(void)
{
    ...
    if (app_ai_is_setup_complete())
    {
...
#endif
        {
            ...
            if (app_ai_is_use_thirdparty())
            {
                app_ai_if_thirdparty_event_handle(THIRDPARTY_FUNC_KWS, THIRDPARTY_AI_CONNECT);
            }
        }
    }
